...
import { LayoutGrid, PlusCircle, PieChart, Settings, Store, Heart } from 'lucide-react';
function App() {
  const location = useLocation();
  const NavItem = ({ to, icon: Icon, label }) => {
    const isActive = location.pathname === to || (to === '/inventory' && location.pathname.startsWith('/inventory') && location.pathname !== '/inventory/create');
    const isActuallyActive = location.pathname === to || (to === '/inventory' && location.pathname.startsWith('/inventory') && !location.pathname.includes('/create'));
    return (
      <Link 
        to={to} 
  ...
  return (
    <div className="min-h-screen flex bg-[var(--color-surface)]">
      ...
      <aside className="w-72 p-6 h-screen sticky top-0 hidden lg:flex flex-col bg-white/50 backdrop-blur-xl border-r border-pink-50">
        ...
        <nav className="flex-1 space-y-2 overflow-y-auto pr-2 custom-scrollbar pt-4">
          <NavItem to="/gallery" icon={Store} label="Gallery" />
          <NavItem to="/favorites" icon={Heart} label="Favorites" />
          <NavItem to="/inventory" icon={LayoutGrid} label="Admin Panel" />
        </nav>
        <div className="mt-6 p-4 bg-primary-light rounded-[2rem] relative overflow-hidden group shrink-0">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <main className="p-4 md:p-8 lg:p-12 overflow-x-hidden">
          <Routes>
